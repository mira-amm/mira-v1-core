mod success {
    ...
    async fn test_burn() {
        ...
        let Asset { id: lp_asset_id, amount: lp_amount } = mint(&amm, pool_id, wallet.address().into()).await.value;
        ...
        let (asset_0_out, asset_1_out) = burn(
            &amm,
            pool_id,
            wallet.address().into(),
            lp_asset_id,
        ...
        assert_eq!(
            wallet.get_asset_balance(&lp_asset_id).await.unwrap(),
            40_000
        );
...
mod revert {
    ...
    async fn test_burn_zero_amount() {
        ...
        wallet.force_transfer_to_contract(&amm_address, 1_020_100, token_0_id, TxPolicies::default()).await.unwrap();
        wallet.force_transfer_to_contract(&amm_address, 10_000, token_1_id, TxPolicies::default()).await.unwrap();
        let Asset { id: lp_asset_id, amount: _ } = mint(&amm, pool_id, wallet.address().into()).await.value;
        burn(
            &amm,
            pool_id,
            wallet.address().into(),
